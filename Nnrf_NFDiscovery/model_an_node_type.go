...
func (v *AnNodeType) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range AllowedAnNodeTypeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid AnNodeType", value)
}
...
func NewAnNodeTypeFromValue(v string) (*AnNodeType, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for AnNodeType: valid values are %v", v, AllowedAnNodeTypeEnumValues)
	}
}
