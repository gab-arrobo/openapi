...
func (v *FailureCode) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range AllowedFailureCodeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid FailureCode", value)
}
...
func NewFailureCodeFromValue(v string) (*FailureCode, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for FailureCode: valid values are %v", v, AllowedFailureCodeEnumValues)
	}
}
