...
func (v *UeAuth) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range AllowedUeAuthEnumValues {
	...
	return fmt.Errorf("%+v is not a valid UeAuth", value)
}
...
func NewUeAuthFromValue(v string) (*UeAuth, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for UeAuth: valid values are %v", v, AllowedUeAuthEnumValues)
	}
}
